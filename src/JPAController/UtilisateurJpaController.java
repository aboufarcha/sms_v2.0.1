...
public class UtilisateurJpaController implements Serializable {
    ...
    public void edit(Utilisateur utilisateur) throws NonexistentEntityException, Exception {
        ...
        try {
            ...
            if (msg == null || msg.length() == 0) {
                Integer id = utilisateur.getIDAdmin();
    ...
    public Utilisateur findUtilisateurByLogin(String email, String password){
            EntityManager em = getEntityManager();
            TypedQuery<Utilisateur> query = em.createQuery("select u from  Utilisateur u where u.email = :email and u.password = :password", Utilisateur.class);
            query.setParameter("email", email);
            query.setParameter("password", Cryptographie.Encrypting(password));
